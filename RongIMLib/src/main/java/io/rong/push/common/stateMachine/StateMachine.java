...
public class StateMachine {
    ...
    public final Collection<LogRec> copyLogRecs() {
        ...
        SmHandler smh = this.mSmHandler;
        if (smh != null) {
            Iterator var3 = smh.mLogRecords.mLogRecVector.iterator();

            while(var3.hasNext()) {
                LogRec lr = (LogRec)var3.next();
                vlr.add(lr);
            }
        }
    ...
    public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        for(int i = 0; i < this.getLogRecSize(); ++i) {
            LogRec logRec = this.getLogRec(i);
            if (logRec != null) {
                pw.printf(" rec[%d]: %s\n", i, logRec.toString());
            }
    ...
    private static class SmHandler extends Handler {
        ...
        private final void completeConstruction() {
            ...
            while(var2.hasNext()) {
                StateInfo si = (StateInfo)var2.next();
                int depth = 0;
    ...
    private static class LogRecords {
        ...
        synchronized LogRec get(int index) {
            ...
            return nextIndex >= this.size() ? null : (LogRec)this.mLogRecVector.get(nextIndex);
        }
